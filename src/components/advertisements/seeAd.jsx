...
const { getAd, getFavorites, deleteFavorite, addFavorite, markAsSold,
  markAsNotSold, markAsReserved, markAsUnreserved, getEmail, sendEmail,
} = apiCall();
  ...
  useEffect(() => {
    const getFavAds = async () => {
      ...
      if (userFavs.length > 0) {
        const map = userFavs.map((x) => x._id);
...
&nbsp;
            ...
            <p>
              <strong>
                {props.intl.formatMessage({ id: 'seeAd.description' })}
                :
              </strong>
              <br />
